...
class Client247SupabaseUser extends BaseAuthUser {
  Client247SupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> client247SupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = Client247SupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
